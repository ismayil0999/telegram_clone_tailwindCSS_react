  ...
  return (
      <div className='bg-background1 w-30 h-screen overflow-y-scroll relative brand  max-sm:w-full'>
       <div className='max-sm:w-full flex justify-between bg-background1 text-white items-center box-border pl-3 pr-4 pt-2 h-14 w-30 gap-5 fixed z-10'>
        {menu===true ? <CloseIcon fontSize='large' className='ease-linear text-color2' onClick={()=>{dispatch(closemenu())}}/> :  <MenuIcon fontSize='large' className='text-color2' onClick={()=>{dispatch(openmenu())}}/> }
     ...
     <div className='mt-14 brand overflow-x-hidden relative'>
         {loading===true ? <CircularProgress className='progress'/> : contacts.map(contact=>{
             return <NavLink to={`/chat/${contact.name}`}>
               <div ref={stil} className='flex items-center h-20 box-border pl-3 gap-4 hover:bg-hover'>
