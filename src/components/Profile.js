...
import { edituser,closeditor,editprofile,closemenu,changeimg } from '../redux/data';
...
export default function Profile() {
  ...
  return (
    <div className='absolute top-0 left-0 w-30 flex flex-col pb-12 box-border h-screen brand overflow-y-scroll z-50 bg-background1 max-sm:w-full'>
      <div className='flex justify-between box-border pl-2 pr-4 pt-4 pb-4 items-center text-white gap-8'>
        <ArrowBackIcon className='text-color2 cursor-pointer' onClick={()=>{dispatch(closemenu())}}/>
...
<EditIcon className='text-color2 cursor-pointer' onClick={()=>{dispatch(edituser())}}/>
      ...
      <div className="w-full h-full bg-cover bg-center" style={{backgroundColor:"grey"}} ref={image}></div>
...
<div class="relative h-11 w-full min-w-[200px]">
  ...
  <input type="file" ref={source} onChange={()=>{dispatch(changeimg({source:source.current.files[0],img:image.current}))}}></input>
 </div>
